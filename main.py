...
print("RPC Host:", RPC_HOST)
print("Starting load test with queued sender wallets...")

test_start_time = None

# Determine if this is a master, worker, or standalone run.
...
if is_worker:
    ...
    print(
        ...
        "  locust --master -f main.py\n"
        "Or run Locust in single-core mode with Prometheus enabled:\n"
        "  locust -f main.py --html=locust_report.html\n"
    )
else:
...
# Event listeners for test start/stop.
@events.test_start.add_listener
def on_test_start(environment, **kwargs):
    global test_start_time
    test_start_time = datetime.now()
    initialize_transaction_log()
    remove_old_report()  # Delete any old report before starting.
...
def on_test_stop(environment, **kwargs):
    ...
    # Rename transaction log file on all nodes.
...
# User class to define the load test.
